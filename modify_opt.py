...
def get_user_choice(opt_files):
    print("\nList of .opt files in the directory:")
    for i, opt_file in enumerate(opt_files):
        print(f"{i+1}. {opt_file}")

    selected_option = input("Enter the indices of files to edit (comma-separated) or 'all' to edit all: ")
    if selected_option.lower() == 'all':
        return opt_files
    else:
        selected_indices = selected_option.split(',')
        selected_files = [opt_files[int(index)-1] for index in selected_indices]
        return selected_files

def edit_selected_files(selected_files):
    for selected_file in selected_files:
        update_opt_file(selected_file)

        # Remove duplicates after modification
        remove_duplicates(selected_file)

        # Sort lines alphabetically
        sort_lines_alphabetically(selected_file)

        print(f"{selected_file} modified successfully.")


if __name__ == "__main__":
    if len(sys.argv) == 2 and sys.argv[1].endswith('.opt'):
        # If a single .opt file is provided as a command-line argument, process it
        opt_file = sys.argv[1]
        ...
        # Otherwise, proceed with the interactive options
        directory = os.path.dirname(os.path.realpath(__file__))
        opt_files = find_opt_files(directory)

        if not opt_files:
            print("No .opt files found in the directory. Exiting.")
            input("Press ENTER to close this window.")
            sys.exit()

        user_choice = get_user_choice(opt_files)

        if user_choice:
            edit_selected_files(user_choice)
        input("Press ENTER to close this window.")