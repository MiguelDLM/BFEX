...
class VIEW3D_OT_ExportSensitivityAnalysisOperator(Operator):
    ...
    def execute(self, context):
        ...
        # Encontrar los vértices más cercanos a los puntos originales
        contact_point1 = Vector((float(Contact_point1.split(',')[0]), float(Contact_point1.split(',')[1]), float(Contact_point1.split(',')[2])))
        nearest_contact_point1 = find_nearest(tree, contact_point1)
        nearest_contact_point1 = nearest_contact_point1[0]
        nearest_contact_point1 = f"{nearest_contact_point1[0]:.6f}, {nearest_contact_point1[1]:.6f}, {nearest_contact_point1[2]:.6f}"
        
        constraint_point1 = Vector((float(Constraint_point1.split(',')[0]), float(Constraint_point1.split(',')[1]), float(Constraint_point1.split(',')[2])))
        nearest_constraint_point1 = find_nearest(tree, constraint_point1)
        nearest_constraint_point1 = nearest_constraint_point1[0]
        nearest_constraint_point1 = f"{nearest_constraint_point1[0]:.6f}, {nearest_constraint_point1[1]:.6f}, {nearest_constraint_point1[2]:.6f}"


        if Contact_point2:
            contact_point2 = Vector((float(Contact_point2.split(',')[0]), float(Contact_point2.split(',')[1]), float(Contact_point2.split(',')[2])))
            nearest_contact_point2 = find_nearest(tree, contact_point2)
            nearest_contact_point2 = nearest_contact_point2[0]
            nearest_contact_point2 = f"{nearest_contact_point2[0]:.6f}, {nearest_contact_point2[1]:.6f}, {nearest_contact_point2[2]:.6f}"
            contact_pts = [[float(coord) for coord in nearest_contact_point1.split(",")], [float(coord) for coord in nearest_contact_point2.split(",")]]
        else:
            contact_pts = [[float(coord) for coord in nearest_contact_point1.split(",")]]

        if Constraint_point2:
            constraint_point2 = Vector((float(Constraint_point2.split(',')[0]), float(Constraint_point2.split(',')[1]), float(Constraint_point2.split(',')[2])))
            nearest_constraint_point2 = find_nearest(tree, constraint_point2)
            nearest_constraint_point2 = nearest_constraint_point2[0]
            nearest_constraint_point2 = f"{nearest_constraint_point2[0]:.6f}, {nearest_constraint_point2[1]:.6f}, {nearest_constraint_point2[2]:.6f}"

           
            constraint_pts = [
                f"p['axis_pt1'] = {[float(coord) for coord in nearest_constraint_point1.split(',')]}\n",
                f"    p['axis_pt2'] = {[float(coord) for coord in nearest_constraint_point2.split(',')]}\n"
            ]
            constraint_axes = f"'axis_pt1': [{constraint_axes_cp1}],\n\t\t'axis_pt2': [{constraint_axes_cp2}]"
